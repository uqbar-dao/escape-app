...
import AnyAscii from '../../node_modules/any-ascii/any-ascii';
...
export const getModuleIcon = (mod: GraphModule): string => {
  // TODO: find icons for link, post, and publish
  if (mod === 'link') {
...
export function dmCounterparty(resource: string) {
  ...
  return ship === `~${global.ship}` ? `~${name.slice(4)}` : ship;
}
...
export function cite(ship: string, useOldFormat = false): string {
  let patp = ship,
...
export const svgDataURL = (svg: any) => 'data:image/svg+xml;base64,' + btoa(svg);

export const svgBlobURL = (svg: any) => URL.createObjectURL(new Blob([svg], { type: 'image/svg+xml' }));
...
export function createStorageKey(name: string, ship: string): string {
  return `~${deSig(ship)}/landscape/${name}`;
}
