...
import Urbit, { FatalError } from '../api';
import { SubscriptionRequestInterface } from '@urbit/http-api';
import { Poke } from '@urbit/api';
import { clearStorageMigration, createStorageKey, storageVersion } from '../util/landscape';
...
export const stateStorageKey = (stateName: string) => {
  const key = createStorageKey(`${stateName}State`, global.ship);
  stateStorageKeys = [...new Set([...stateStorageKeys, key])];
  return key;
};

global.clearStates = async () => {
  return Promise.all(stateStorageKeys.map((key) => storage.remove({ key })));
};
...
export function createSubscription(app: string, path: string, e: (data: any) => void): SubscriptionRequestInterface {
  ...
  const request = {
    ...
    path,
    event: e,
    err: () => console.warn('ERROR WITH SUBSCRIPTION'),
    quit: () => {
...
export const createState = <T extends {}>(
  ...
  name: stateStorageKey(name, global.ship),
