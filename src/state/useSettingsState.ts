...
import { useShortcut as usePlainShortcut } from '../util/shortcutContext';
import {
  BaseState,
  createState,
  createSubscription,
...
const useSettingsState = createState<SettingsState>(
  ...
  (set, get) => ({
    ...
    getAll: async () => {
      const { desk } = await global.api.scry(
        getDeskSettings(window.desk)
      );
      get().set((s) => {
    ...
    putEntry: async (bucket: string, entry: string, value: Value) => {
      const poke = putEntry(window.desk, bucket, entry, value);
